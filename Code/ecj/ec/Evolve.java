 ...
 */
    ...
    public static void run(String[] conf, String alg, String problem)
    {
        ...
        state = possiblyRestoreFromCheckpoint(conf);
        ...
        if (state != null)  // loaded from checkpoint
            ...
            try
                ...
                conf = state.runtimeArguments;                          // restore runtime arguments from checkpoint
        ...
        parameters = loadParameterDatabase(conf);
        ...
        // it's usually small.
        long runTime[]=new long[numJobs]; // PTT
        for(int job = currentJob ; job < numJobs; job++)
            ...
            // try
                {
                runTime[job]= System.currentTimeMillis();
                ...
                state.nameJob =job; // PTT
                state.nameProb=problem; // PTT
                state.nameAlg=alg; // PTT
                ...
                state.runtimeArguments = conf;                              // stick the runtime arguments in our storage
                ...
                if (numJobs > 1)                                                    // only if iterating (so we can be backwards-compatible),
                   ...
                   if((state.nameAlg=="sfgp")||(state.nameAlg=="sf_mssc")){
                       jobFilePrefix = "/home/thuongpt/ECJ-Regression/out/" + alg + "/Case1_TTest/" + problem + "/job." + job + ".";
                       //jobFilePrefix = "E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression/out/" + alg + "/Case1/" + problem + "/job." + job + ".";
                   }else{
                        jobFilePrefix = "/home/thuongpt/ECJ-Regression_server/out/" + alg + "/" + problem + "/job." + job + ".";
                        //jobFilePrefix = "E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression/out/" + alg + "/" + problem + "/job." + job + ".";
                   }
                ...
                runTime[job]=System.currentTimeMillis()-runTime[job];
                }
            /*
    	...
    	// xuat ra file runTime
        writeRunTime(problem, alg, runTime);
    }
    public static void writeRunTime(String problem, String alg, long [] runTime){
    try {
      int i;
      ...
      if((alg=="sfgp")||(alg=="sf_mssc")){
          //fName="E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression_server/out/"+alg+"/Case1_TTest/"+problem+"/timeOfRuns.res";
          fName="/home/thuongpt/ECJ-Regression/out/" + alg + "/Case1_TTest/"+problem+"/timeAllRuns.res"; 
      }else{
        //fName="E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression_server/out/"+alg+"/"+problem+"/timeOfRuns.res"; 
        fName="/home/thuongpt/ECJ-Regression/out/" + alg + "/"+problem+"/timeAllRuns.res"; 
      }
    
      out= new BufferedWriter(new FileWriter(fName));
      out.write("Tong thoi gian chay theo tung run: ");
      out.newLine();
      
      for(i=0; i<runTime.length;i++){
          out.write(new Long(runTime[i]).toString());
          out.newLine();
      }
      out.close();
    }catch(Exception e ) {
    ...
    public static void main(String[] args)
        {
    	//phishing
    	//String param_file = "/home/hanu/pta/projects/ECJ21/ecj/ec/app/regression/benchmark.params";
    	//boolean
    	//String param_file = "/home/hanu/pta/projects/ECJ21/ecj/ec/app/parity/parity-lgx.params";
		
    	//String[] conf = {"-file", param_file};
		//run(conf, "lgx", "parity-7");

    	//benchmarks
    	String[] conf = {"-file", "/home/pta/projects/ECJ21/ecj/ec/app/phishing/phishing.params", 
    			//"-p", "stat=ec.app.phishing.PhishingStatistics", 
    			"-p", "stat.gather-full=true",
    			//"-p", "gp.tree.print-style=c"
    			};
...
//				"vladislavleva-2",
				...
				"F4", 
                    ...
                    //"Korns14",
                    //"Korns15",
                               // "Census6", // da co ket qua
                               // "No2" ,    // da co ket qua
                               // "horse_races", // da co ket qua
                               // "housing",  //da co ket qua
                               // "winequality_red",//da co ket qua
                               // "parkinsons_updrs", //xong 1 bai
                               // "data",  // da co ket qua
                               // "winequality_white",//da co ket qua
				...
				"F11",  // 
                                //"bupa", // QRF loi
                      // "UCI_GreenhouseGasObservingNetwork",   
                      // "UCI_GasSensorArrayDrift_batch5",
                     //  "UCI_GasSensorArrayDrift_batch1",
                    //   "StatLib_bank_32nh",
                    //   "StatLib_bank_32nm",
                       
                     //  "UCI_GasSensorArrayDrift_batch2",
                     //  "UCI_GasSensorArrayDrift_batch3", //xong la xong
                       
                     //  "UCI_GasSensorArrayDrift_batch10",
                       // "casp",
                       //"wpbc",
    	...
    	for(String problem_name : problems)
    	{
    		//String problem_name = "trial_0_fold_" + String.valueOf(i);
    		//String problem_name = "nguyen-4";
    		
    		for(String alg: algs)
    		{
    			String param_file = "/home/thuongpt/ECJ-Regression/params/" + alg + "/" + problem_name +".params";
    			//String param_file = "E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression_server/params/" + alg + "/" + problem_name +".params";
                        System.out.println(param_file);
    			conf[1] = param_file;

    			run(conf, alg, problem_name);
    		}
    	}
        System.exit(0);
        }
