 ...
 * testing data and functions for the following problems:
 ...
 by Nguyen Quang Uy, Nguyen Xuan Hoai, Michael l, R.I. McKay, Edgar Galv ́an-L ́opez.  These are known as
...
public class Benchmarks extends GPProblem implements SimpleProblemForm
    ...
    public static final int SEXTIC = 53;
    public static final int SEPTIC = 54;
    public static final int NONIC = 55;
    public static final int R1 = 56;
    public static final int R2 = 57;
    public static final int R3 = 58;
    
    //UCI
    public static final int BODYFAT = 59;
    public static final int CENSUS = 60;
    public static final int CONCRETE = 61;
    public static final int FOREST = 62;
    public static final int MG = 63;
    public static final int NO = 64;
    public static final int POLLEN = 65;
    public static final int PYRIM = 66;
    public static final int SPACE_GA = 67;
    public static final int SR2N = 68;
    public static final int SR7N = 69;
    public static final int SR8 = 70;
    public static final int SR10N = 71;
    public static final int TRIAZINES = 72;
    public static final int WDBC = 73;
    public static final int WINEQUALITY = 74;
    ...
    public static final String names[] = 
        ...
        //from UCI, Statlib
        "bodyfat", "census", "concrete", "forest", "mg", "no", "pollen", "pyrim", "space_ga", "sr2n", "sr7n", "sr8", "sr10n", "wdbc", "winequality",
        }; 
    ...
    public static final String fs[] = 
        ...
        "koza1", "koza1", "koza1", "koza1", "koza1", "koza1", //sextic, septic, nonic, r1, r2, r3
        "fs14", "fs6", "fs8", "fs12", "fs6", "fs7", "fs4", "fs27", "fs6", "fs1", "fs4", "fs2", "fs2", "fs30", "fs11",
        //"fs6", "fs14", "fs27", "fs60"
        };
    ...
    public static final String fs_vars[][] = 
        {
        { },
            { "koza1", "keijzer1", "vladislavleva-c1", "fs1" },
            { "koza2", "keijzer2", "vladislavleva-a2", "vladislavleva-b2", "vladislavleva-c2", "fs2" },
            { "koza3", "keijzer3", "vladislavleva-a3", "fs3" },
            { "fs4"},
            { "korns5", "vladislavleva-a5", "fs5" },
            //6
            {"fs6"},
            //7
            {"fs7"},{"fs8"},{"fs9"},{"fs10"},{"fs11"},{"fs12"},{"fs13"},
            //14
            {"fs14"},
            //15-26
            {"fs15"},{"fs16"},{"fs17"},{"fs18"},{"fs19"},{"fs20"},{"fs21"},{"fs22"},{"fs23"},{"fs50"},{"fs25"},{"fs26"},
            //27
            {"fs27"}, {"fs28"}, {"fs29"}, {"fs30"}
    ...
    public void writeFile(EvolutionState state, int benchmark, String training, double[][] input)
    {
    	try {
    		
            PrintStream out = new PrintStream(new File(names[benchmark]+"."+training+".in"));
            out.println(input[0].length);
            ...
            for (int i = 0; i < input.length; i++) {
            	...
            	line += func(state, input[i], benchmark);
            	// Lam nhieu Gaussian
            	//double f = func(state, input[i], benchmark);
            	//f = f + state.random[0].nextGaussian() * 0.2;
            	//line += f;
            	out.println(line);
            }
            out.close();
    ...
    public double[][] trainPoints(EvolutionState state, int benchmark, int threadnum)
        ...
        switch(benchmark)
            ...
            case NGUYEN9:
            case NGUYEN10:
                return generateRandomSamples(state, new double[] { 0, 0 }, new double[] { 1, 1 }, 20, threadnum);
            ...
            case KORNS14:
            case KORNS15:
                return generateRandomSamples(state, new double[] { -1, -1, -1, -1, -1 }, new double[] { 1, 1, 1, 1, 1 }, 20, threadnum);  // 10000 !!
            ...
            case KEIJZER4:
            	...
            	return generateRandomSamples(state, 0, 10, 20, threadnum);
            ...
            case KEIJZER14:
            case KEIJZER15:
                return generateRandomSamples(state, new double[] { -1, -1 }, new double[] { 1, 1 }, 20, threadnum);
...
//            	return generateIntervalSpacedSamples(state, new double[] { 0.05, 0.05 }, new double[] { 10, 10.05 }, new double[] { 0.1, 2 }, threadnum);
            ...
            case VLADISLAVLEVA4:
                return generateRandomSamples(state, new double[] { 0.05, 0.05, 0.05, 0.05, 0.05}, new double[] { 6.05, 6.05, 6.05, 6.05, 6.05 }, 500, threadnum);
    ...
    public double[][] testPoints(EvolutionState state, int benchmark, int threadnum, double[][] trainpoints)
        ...
        switch(benchmark)
            ...
            case KEIJZER4:
            	...
            	return generateRandomSamples(state, 0, 1, 500, threadnum);
            ...
            case VLADISLAVLEVA4:
                return generateRandomSamples(state, new double[] { -0.25, -0.25, -0.25, -0.25, -0.25}, new double[] { 6.35, 6.35, 6.35, 6.35, 6.35 }, 500, threadnum);
            ...
            case R2:
            case R3:
//            	return generateIntervalSpacedSamples(state, -1, 1, 20, threadnum);
                    return generateRandomSamples(state, -1, 1, 20, threadnum);
    ...
    public double func(EvolutionState state, double[] xs, int benchmark) throws IllegalArgumentException
        ...
        switch(benchmark)
            ...
            case SEPTIC:
            	return x*x*x*x*x*x*x - 2.0 * x*x*x*x*x*x + x*x*x*x*x - x*x*x*x + x*x*x - 2.0 * x*x + x;
            case NONIC:
            	return x*x*x*x*x*x*x*x*x + x*x*x*x*x*x*x*x + x*x*x*x*x*x*x + x*x*x*x*x*x + x*x*x*x*x + x*x*x*x + x*x*x + x*x + x;
            case R1:
            	return ((x+1.0)*(x+1.0)*(x+1.0))/(x*x - x + 1);
            case R2:
            	return (x*x*x*x*x - 3.0 * x*x*x + 1)/(x*x + 1);
            case R3:
...
/////// Computation of delta error between expected value and provided value
    ...
    protected final static double PROBABLY_ZERO = 1.11E-15;
    protected final static double BIG_NUMBER = 1.0e15;                // the same as lilgp uses
...
//        String problem = state.parameters.getString(base.push(P_PROBLEM_TYPE), null);
        ...
        //Write training files and testing files
        
/*        for (int i = 0; i < names.length; i++)
        {
        	double[][] tps = trainPoints(state, i, 0);
        	//for training
        	writeFile(state, i, "training", tps);
        	//for testing
        	writeFile(state,i, "testing", testPoints(state, i, 0, tps));
        }
*/
        if (problem == null)
            {
...
///// Evaluation.  evaluate(...) uses training cases, and describe(...) uses testing cases
    ...
    public void evaluate(EvolutionState state, Individual ind, int subpopulation, int threadnum)
		...
		//error =error/(Math.abs(input.x)+Math.abs(trainingOutputs[y]));
		//sum += error;
		
                }
                
            	// RMSE
		sum = sum/trainingInputs.length;
		float fit = (float) Math.sqrt(sum);
    ...
    public void describe(EvolutionState state, Individual ind, int subpopulation, int threadnum, int log)
        ...
        GPIndividual t=((GPIndividual)ind);
        t.valueArr=new double[testingInputs.length];
	    ...
	    // CD: Mean Canberra Distance
	    // error=error/(Math.abs(input.x)+Math.abs(testingOutputs[y]));  
           //  sum+=error;
	...
	//float fit = (float)sum;
        ...
        state.output.print("" + f.standardizedFitness() + " "+ ind.size()+" ", log);  

        computeMeanAndVar_FirstWay_NoBootstrap(state, ind, subpopulation, threadnum, log,state.inputTrainThg, "tAlpha_div2"); //ok
        //computeMeanAndVar_FirstWay(state, ind, subpopulation, threadnum, log);
        //print tree
...
*/        //f.printFitnessForHumans(state, log);
        ...
        /* 4 th Method (PTT): below is common for GP systems
         * - Do not bootrap sampling to compute mean and var err in the formula prediction interval
         * - Use mean and std of residual error sample S as bias and variance of population which are assummed Gaussian distridution
         */
        public void computeMeanAndVar_FirstWay_NoBootstrap(EvolutionState state, Individual ind, int subpopulation, int threadnum, int log, RegressionData input1, String typeTAlpha){
           if(typeTAlpha=="tAlpha_div2"){
               // 100 points
               if((state.nameProb=="F2")||(state.nameProb=="bupa")||(state.nameProb=="Census6")||(state.nameProb=="No2")){// 100 fitcase, bac tu do n-1=99
                   state.freeDegree=1.987; 
               }
               // 300 points
               if(state.nameProb=="housing"){// 300 fitcase, bac tu do n-1
                   state.freeDegree=1.984; 
               } 
               // 300 points
               if(state.nameProb=="F4"){// 300 fitcase, bac tu do n-1
                   state.freeDegree=1.972; 
               } 
               // 50 points
               if((state.nameProb=="F5")||(state.nameProb=="F7")){
                   state.freeDegree=2.011; 
               }
               // 20 points
               if((state.nameProb=="F8")||(state.nameProb=="F9")||(state.nameProb=="F10")||(state.nameProb=="F11")||(state.nameProb=="horse_races")){// 300 fitcase, bac tu do n-1=99
                   state.freeDegree=2.093; 
               }
               // > 600 points
               if((state.nameProb=="parkinsons_updrs")||(state.nameProb=="data")){
                   state.freeDegree=1.960; 
               }
           }else{  // tAlpha: one side
               // 100 points
               if((state.nameProb=="F2")||(state.nameProb=="bupa")||(state.nameProb=="Census6")||(state.nameProb=="No2")){// 100 fitcase, bac tu do n-1=99
                   state.freeDegree=1.662; 
               }
               // 300 points
               if(state.nameProb=="housing"){// 300 fitcase, bac tu do n-1
                   state.freeDegree=1.660; 
               } 
               // 300 points
               if(state.nameProb=="F4"){// 300 fitcase, bac tu do n-1
                   state.freeDegree=1.653; 
               } 
               // 50 points
               if((state.nameProb=="F5")||(state.nameProb=="F7")){
                   state.freeDegree=1.677; 
               }
               // 20 points
               if((state.nameProb=="F8")||(state.nameProb=="F9")||(state.nameProb=="F10")||(state.nameProb=="F11")||(state.nameProb=="horse_races")){// 300 fitcase, bac tu do n-1=99
                   state.freeDegree=1.729; 
               }
               // > 600 points
               if((state.nameProb=="parkinsons_updrs")||(state.nameProb=="data")){
                   state.freeDegree=1.645; 
               }
           }
           double[] errorsL2 = new double[trainingInputs.length];  // nguyen ban nhu RMSE
           double[] errorsL1 = new double[trainingInputs.length]; 
           double sumL1=0;
           double sumL2=0;
            for (int y=0;y<trainingInputs.length;y++)
                {
                    currentValue = trainingInputs[y];
                    ((GPIndividual)ind).trees[0].child.eval(
                        state,threadnum,input1,stack,((GPIndividual)ind),this);
                    //store semantic of y-th element
                    ((GPIndividual)ind).trees[0].semanticTraining[y] = input1.x;
                
                    double error = error(input1.x, trainingOutputs[y]);
                    //System.out.print(error+", "); //PTT
                    errorsL2[y]=error * error; 
                    errorsL1[y]=error;
                ...
                double biasL2, biasL1,biasMLS,biasRMSE,biasLS;
                // can loi chia n
                //biasL2= Math.sqrt(sumL2); // can bac hai cua loi
                // L1: fabs: loi tuyet doi chia n
                biasL1= sumL1/trainingInputs.length; // fabs
                // LeastSquared loss function: Tong binh phuong loi chia n
                biasMLS = sumL2/trainingInputs.length;
                // RMSE
                biasRMSE = Math.sqrt(sumL2/trainingInputs.length); // RMSE
                // tong binh phuong khong chia n
                //biasLS = sumL2; 
                // Can n
                //biasCanN[i] = Math.sqrt(biasCanN[i]); 
                
              
               
                for(int y = 0; y < trainingInputs.length; y++)
                {
                    ((GPIndividual)ind).varL1_cach2 += Math.pow(errorsL1[y] - biasL1, 2);
                    ((GPIndividual)ind).varMeanLeastSquare += Math.pow(errorsL2[y] - biasMLS, 2);
                    ((GPIndividual)ind).originalVar += Math.pow(Math.sqrt(errorsL2[y]) - biasRMSE, 2);
                    
                }
               
                ((GPIndividual)ind).varL1_cach2 = ((GPIndividual)ind).varL1_cach2 /(trainingInputs.length- 1);
                ((GPIndividual)ind).varMeanLeastSquare = ((GPIndividual)ind).varMeanLeastSquare/(trainingInputs.length- 1);
                ((GPIndividual)ind).originalVar = ((GPIndividual)ind).originalVar/(trainingInputs.length- 1);
            
                
               // System.out.println("----------GP: var L1_Cach2:"+((GPIndividual)ind).varL1_cach2);
               // System.out.println("----------GP: var RMSE:"+((GPIndividual)ind).originalVar);
               // System.out.println("----------GP: var mean least squared:"+((GPIndividual)ind).varMeanLeastSquare);
                //  System.out.println("----------GP: var_least squared:"+((GPIndividual)ind).var_LeastSquare)
               
                ((GPIndividual)ind).meanL1_cach2=biasL1;
                ((GPIndividual)ind).meanMeanLeastSquare=biasMLS;
                ((GPIndividual)ind).originalMean=biasRMSE;
      }
        // lay bootstrap de tinh mean and variance
        /* 1st method:
        *   - Buoc 1: Lay mo hinh tot nhat ung voi cac phuong phap GP,, BVGP, SFGP, 
        *   - Buoc 2: tinh loi residual(theo cong thuc RMSE, Mean Least Square, fabs) cho tung fitcase trong tap train su dung  mo hinh tot nhat hoc duoc: ket qua la 1 sample_err: S
        *   - Buoc 3: sinh ra B mau bootstrap tu S
        *   - Buoc 4: Xac dinh mean va var cua S
        *   - Buoc 5: Ap dung mean va var cho cong thuc tinh khoang tuong ung
        */ 
        public static int NUMBOOT=1000;
        public void computeMeanAndVar_FirstWay(EvolutionState state, Individual ind, int subpopulation, int threadnum, int log){
            RegressionData input = (RegressionData)(this.input);
            input.x=0;
            // if((state.generation==state.numGenerations-1)){
         //  System.out.println("3. Test trong method computeMeanAndVar_FirstWay: "+input.x);
            ...
            for (int y=0;y<trainingInputs.length;y++)
                ...
                double[] biasL2_cach1 = new double[NUMBOOT];
                double[] biasL1_cach2 = new double[NUMBOOT];
                double[] biasMeanLeastSquare=new double[NUMBOOT];
                double[] biasOriginal=new double[NUMBOOT];
                double[] bias_LeastSquare=new double[NUMBOOT];
                for (int i = 0; i < NUMBOOT; i++)
                {
                    ...
                    biasL2_cach1[i] = 0;
                    biasL1_cach2[i] = 0;
                    biasMeanLeastSquare[i]=0;
                    bias_LeastSquare[i]=0;
                    biasOriginal[i]=0; // RMSE
                    for(int j = 0; j < trainingInputs.length; j++)
                    {
                        biasL2_cach1[i] += errorsL2[dataSet[index][j]];
                        biasL1_cach2[i] += errorsL1[dataSet[index][j]];
                        biasMeanLeastSquare[i] += errorsL2[dataSet[index][j]];
                        bias_LeastSquare[i] += errorsL2[dataSet[index][j]];
                        biasOriginal[i] += errorsL2[dataSet[index][j]];
                    }
                     // can loi chia n
                    biasL2_cach1[i] = Math.sqrt(biasL2_cach1[i]); // can bac hai cua loi
                    // L1: fabs: loi tuyet doi chia n
                    biasL1_cach2[i] = biasL1_cach2[i]/trainingInputs.length; // fabs
                    // LeastSquared loss function: Tong binh phuong loi chia n
                    biasMeanLeastSquare[i] = biasMeanLeastSquare[i]/trainingInputs.length;
                    // RMSE
                    biasOriginal[i] = Math.sqrt(biasOriginal[i]/trainingInputs.length); // RMSE
                    // tong binh phuong khong chia n
                    bias_LeastSquare[i] = bias_LeastSquare[i]; 
                    // Can n
                    //biasCanN[i] = Math.sqrt(biasCanN[i]); 
                }
                for (int i = 0; i < NUMBOOT; i++){
                    ((GPIndividual)ind).meanL1_cach2+= biasL1_cach2[i];
                    ((GPIndividual)ind).meanL2_cach1+=biasL2_cach1[i];
                    ((GPIndividual)ind).meanMeanLeastSquare+=biasMeanLeastSquare[i];
                    ((GPIndividual)ind).originalMean+=biasOriginal[i];
                    ((GPIndividual)ind).mean_LeastSquare+=bias_LeastSquare[i];
                }
                ((GPIndividual)ind).meanL1_cach2=((GPIndividual)ind).meanL1_cach2/NUMBOOT;
                ((GPIndividual)ind).meanL2_cach1=((GPIndividual)ind).meanL2_cach1/NUMBOOT;
                ((GPIndividual)ind).meanMeanLeastSquare=((GPIndividual)ind).meanMeanLeastSquare/NUMBOOT;
                ((GPIndividual)ind).originalMean=((GPIndividual)ind).originalMean/NUMBOOT;
                ((GPIndividual)ind).mean_LeastSquare=((GPIndividual)ind).mean_LeastSquare/NUMBOOT;
                for(int i = 0; i < NUMBOOT; i++){
                    ((GPIndividual)ind).varL2_cach1 += Math.pow(biasL2_cach1[i] - ((GPIndividual)ind).meanL2_cach1, 2);
                    ((GPIndividual)ind).varL1_cach2 += Math.pow(biasL1_cach2[i] - ((GPIndividual)ind).meanL1_cach2, 2);
                    ((GPIndividual)ind).varMeanLeastSquare += Math.pow(biasMeanLeastSquare[i] - ((GPIndividual)ind).meanMeanLeastSquare, 2);
                    ((GPIndividual)ind).originalVar += Math.pow(biasOriginal[i] - ((GPIndividual)ind).originalMean, 2);
                    ((GPIndividual)ind).var_LeastSquare += Math.pow(bias_LeastSquare[i] - ((GPIndividual)ind).mean_LeastSquare, 2);
                }
                ((GPIndividual)ind).varL2_cach1 = ((GPIndividual)ind).varL2_cach1/(NUMBOOT- 1);
                ((GPIndividual)ind).varL1_cach2 = ((GPIndividual)ind).varL1_cach2 /(NUMBOOT- 1);
                ((GPIndividual)ind).varMeanLeastSquare = ((GPIndividual)ind).varMeanLeastSquare/(NUMBOOT- 1);
                ((GPIndividual)ind).originalVar = ((GPIndividual)ind).originalVar/(NUMBOOT- 1);
                ((GPIndividual)ind).var_LeastSquare = ((GPIndividual)ind).var_LeastSquare/(NUMBOOT- 1);
                
                // tinh Quantile_0.025, Quantile_0.975 cá»§a phÃ¢n bá»‘ máº«u bootrap
                sortIncrease(biasL2_cach1);
                sortIncrease(biasL1_cach2);
                sortIncrease(biasMeanLeastSquare);
                sortIncrease(biasOriginal);
                sortIncrease(bias_LeastSquare);
                
                ((GPIndividual)ind).lowPre_L1=biasL1_cach2[25];
                ((GPIndividual)ind).highPre_L1=biasL1_cach2[975];
                ((GPIndividual)ind).lowPre_L2=biasL2_cach1[25];
                ((GPIndividual)ind).highPre_L2=biasL2_cach1[975];
                ((GPIndividual)ind).lowPre_RMSE=biasOriginal[25];
                ((GPIndividual)ind).highPre_RMSE=biasOriginal[975];
                ((GPIndividual)ind).lowPre_LS=bias_LeastSquare[25];
                ((GPIndividual)ind).highPre_LS=bias_LeastSquare[975];
                ((GPIndividual)ind).lowPre_MLS=biasMeanLeastSquare[25];
                ((GPIndividual)ind).highPre_MLS=biasMeanLeastSquare[975];
        }
         // sort increase
			...
			int threadnum) 
	{
		...
		double[] semantic = new double[trainingInputs.length];
	
		for(int i = 0; i < trainingInputs.length; i++)
		{
			currentValue = trainingInputs[i];
