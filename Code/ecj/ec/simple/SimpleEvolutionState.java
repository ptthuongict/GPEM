...
public class SimpleEvolutionState extends EvolutionState
    ...
    public void genTreelib(int treelibsize)
    {
    	maxTreelibSize = treelibsize;
        // randomly generate semantic treelib
        SimpleProblemForm problem = (SimpleProblemForm)(this.evaluator.p_problem.clone());
        GPIndividual ind = ((GPIndividual)this.population.subpops[0].individuals[0]).lightClone();
        
        semanticTreelibTraining = new double[maxTreelibSize][problem.getNumOfFitcases()];
        semanticTreelibTesting = new double[maxTreelibSize][problem.getNumOfTestcases()];
        treelib = new GPNode[maxTreelibSize];
        
        for(int i = 0; i < maxTreelibSize; i++)
        {
        	ind.trees[0].buildTree(this, 0);
        	
        	treelib[i] = (GPNode)ind.trees[0].child.clone();
        	
        	semanticTreelibTraining[i] = problem.getSemantic(this, ind.trees[0].child, 0);
        	semanticTreelibTesting[i] = problem.getSemanticTesting(this, ind.trees[0].child, 0);
        	
        	//normalize to [0,1]
        	for(int j = 0; j < semanticTreelibTraining[i].length; j++)
        	{
        		//sigmoid A new implementation of geometric semantic GP applied to predicting pharmacokinetic parameters
        		semanticTreelibTraining[i][j] = 1 / (1 + Math.exp(0-semanticTreelibTraining[i][j]));
        	}
        	
        	for(int j = 0; j < semanticTreelibTesting[i].length; j++)
        	{
        		semanticTreelibTesting[i][j] = 1 / (1 + Math.exp(0-semanticTreelibTesting[i][j]));
        	}
    ...
    public int evolve()
        ...
        //for(int i = 0; i < 500; i++)
        //{
        	//System.out.println(this.population.subpops[0].individuals[i].fitness.fitness());
        //}
