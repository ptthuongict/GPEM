...
#    numcolumns = 26
    ...
    # -5: fittest of the best on gen
    # -4: size of the best on gen
    # -11: fitness of the best on gen
    
    indexColumns = [0, 6, 8, -11, -5, -4]
   
    #SFStatistics
    # 0: generation
    # 6: evaluation time
    # 8: average number of nodes per individual this generation
    # -2 fittest of the best on gen
    # -1 size of the best on gen
    # -6 variance of the best on gen
    # -7 mean of the best on gen
    # -8 fitness of the best on gen
    if operator == 'sfgp':
        indexColumns = [0, 6, 8, -8, -7, -6, -2, -1]
...
#		print fname
                ...
                for gen in gens:
                    ...
                    l[-1].append(float(xs[fitness_idx])) # for sorting
                    l[-1].append(float(xs[evaluationTime_idx]))
                    l[-1].append(float(xs[sizeAvr_idx]))
                    l[-1].append(float(xs[size_idx]))             
                    l[-1].append(float(xs[fittest_idx]))
                    
                # sort l by fitness
                l = sorted(l,key=itemgetter(1))
                
                # run, time (best), size average (best), size of best individual, fittest of best individual, time (median), size average (median), size of median, fittest of median:
                result[i] = result[i] + [i, l[0][2] ,l[0][3],l[0][4], l[0][5], l[NUMGENS/2][2], l[NUMGENS/2][3],l[NUMGENS/2][4] ,l[NUMGENS/2][5]]

                # nhu ecj_thy1.py
               # result[i] = result[i] + [i, l[0][2] , l[0][3] , l[0][5], l[NUMGENS/2][3], l[NUMGENS/2][5]]
...
def getThuongResult():
    ...
    Chi Thuong
    '''
    root = 'E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression_server/out'
    for problem in problems:
        print problem
        result = []
...
#     getTestingFitness(dir)
    ...
    getDataForStatisticalTesting('E:/ThucnghiemNCS_2014/ECJ_All/ECJ-Regression_server/out', operators, problems)
